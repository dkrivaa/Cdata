...
def muni_data(muni, crimegroup4, crimetype4, window_width):
    st.write(f"""
            ### Municipal Data: {muni}
            """)
    ...
    with col1:
        ...
        try:
            ...
            dfi = df.loc[df['Settlement_Council'] == f'{muni}']
    ...
    with col1:
        ...
        try:
            ...
            dfi = df_quarter.loc[df['Settlement_Council'] == f'{muni}']
            total = pd.DataFrame({'y': list(dfi.groupby(dfi['year'])['TikimSum'].sum().values),
                                  'x': (sorted(dfi['year'].unique().tolist()))
                                  })
            total['x'] = total['x'].astype(str)
    ...
    with col2:
        ...
        **{crimegroup4}** by quarter **{muni}**
        ...
        try:
            # Defining the data for the chart of annual tikim
            dfi = df.loc[(df['Settlement_Council'] == f'{muni}') &
                         (df['StatisticCrimeGroup'] == f'{crimegroup4}')]
            total = pd.DataFrame({'y': list(dfi.groupby(dfi['Quarter'])['TikimSum'].sum().values),
    ...
    with col3:
        ...
        **{crimetype4}** by quarter **{muni}**
        ...
        try:
            dfi = df.loc[(df['Settlement_Council'] == f'{muni}') &
                         (df['StatisticCrimeGroup'] == f'{crimegroup4}') &
                         (df['StatisticCrimeType'] == f'{crimetype4}')]
            total = pd.DataFrame({'y': list(dfi.groupby(dfi['Quarter'])['TikimSum'].sum().values),
                                  'x': (sorted(dfi['Quarter'].unique().tolist()))
                                  })
            y_max = 1.05 * max(dfi.groupby(dfi['Quarter'])['TikimSum'].sum().values)
